use super::model::{NewService, Service, UpdateService, ServiceListQuery};
...
pub async fn get_services(app_state: State<AppState>, query: Query<ServiceListQuery>) -> HttpResponse {
    let query = query.into_inner();
    let (filter, options) = construct_find_options_and_filter(query.clone()).unwrap();
    let db = &app_state.db;
