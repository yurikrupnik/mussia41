...
pub async fn get_todo(app_state: Data<AppState>, id: Path<String>) -> HttpResponse {
...
pub async fn get_todos(app_state: Data<AppState>, query: Query<TodoListQuery>) -> HttpResponse {
    let query = query.into_inner();
    let (filter, options) = construct_find_options_and_filter(query.clone()).unwrap();
    let db = &app_state.db;
...
pub async fn delete_todo(app_state: Data<AppState>, id: Path<String>) -> HttpResponse {
    let item_id = id.into_inner();
    let db = &app_state.db;
...
pub async fn create_todo(body: Json<NewTodo>, app_state: Data<AppState>) -> HttpResponse {
...
pub async fn drop_todos(app_state: Data<AppState>) -> HttpResponse {
    let db = &app_state.db;
